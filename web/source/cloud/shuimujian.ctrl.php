...
defined('IN_IA') or exit('Access Denied');
    ...
    $ver = include('shuimujian.php');
    $ver = $ver['ver'];
    $ver = substr($ver,-4);
	$hosturl = urlencode('http://' . $_SERVER['HTTP_HOST']);
	$updatehost = 'http://myr.haoyunfa.net/update.php';
	...
	elseif($_GPC['op'] == 'update'){
		...
		if (strstr($updatenowinfo, 'zip')){			
				...
				if($isgot){				
					...
					$f = fopen('shuimujian.php','w+');
					fwrite($f,$newver);
					fclose($f);
					@unlink(IA_ROOT . '/update.sql');
					deldir($updatedir);
					
					message('已将系统更新到最新版本！',create_url('cloud/shuimujian',array('op'=>'display')),'success');
				}
				else{
				...
				message('痛苦VIP提醒您:请检查授权状态，或者联系QQ 120738008授权！',referer(),'error');
		}
	}	elseif($_GPC['op'] == 'supdate'){
		$op = $_GPC['op'];
		$updatehost = 'http://myr.haoyunfa.net/update.php';
		$updatehosturl = $updatehost . '?a=update&v=' . $ver . '&u=' . $hosturl;
		$updatenowinfo = file_get_contents($updatehosturl);
