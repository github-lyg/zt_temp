...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        ...
        <label class="radio-inline"><input type="radio"  name="data[commissiontype]" value="1" <?php  if($data['commissiontype'] ==1) { ?> checked="checked"<?php  } ?> /> 实际支付</label>
        <label class="radio-inline"><input type="radio"  name="data[commissiontype]" value="2" <?php  if($data['commissiontype'] ==2) { ?> checked="checked"<?php  } ?> /> 订单利润</label>

        <span class="help-block">默认方式: 除运费和会员折扣(或促销折扣)外其他所有费用全部计算佣金<br>实际支付: 只计算实际支付和余额抵扣部分的佣金(包括余额支付)<br>订单利润：订单支付金额减商品成本</span>
        <?php  } else { ?>
        <?php  if(empty($data['commissiontype'])) { ?>默认方式<?php  } ?>
        <?php  if($data['commissiontype'] == 1) { ?>实际支付<?php  } ?>
        <?php  if($data['commissiontype'] == 2) { ?>订单利润<?php  } ?>
        <?php  } ?>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12 fixmore-input-group">
        <?php if(cv('commission.set.edit')) { ?>
        <input type="text" name="data[bonusRate]" class="form-control" value="<?php echo empty($data['bonusRate'])?100:$data['bonusRate']?>"  />
        <span class="help-block">最终佣金 * 佣金比例 = 实际佣金</span>
        <?php  } else { ?>
        <?php  } ?>
    </div>
</div>
