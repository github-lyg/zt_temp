<?php defined('IN_IA') or exit('Access Denied');?><div class="form-group">
    <label class="col-lg control-label">是否开启评论</label>
    <div class="col-sm-9 col-xs-12">
        ...
        <?php  } else { ?>
        <div class='help-block'><?php  if($data['isreply']==1) { ?>显示<?php  } else { ?>关闭<?php  } ?></div>
        <?php  } ?>
    </div>
</div>
<div class="form-group">
    <label class="col-lg control-label">评论敏感词</label>
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('creditshop.cover.edit')) { ?>
            ...
            <div class="help-block">一行一个或用半角英文逗号(,)隔开</div>
        <?php  } else { ?>
            <div class='help-block'><?php  echo $data['desckeyword'];?></div>
        <?php  } ?>
    </div>
</div>

<div class="form-group">
    <label class="col-lg control-label">替换文字</label>
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('creditshop.cover.edit')) { ?>
            <input type='text' class='form-control' name='data[replykeyword]' value="<?php  echo $data['replykeyword'];?>" />
            <div class="help-block">将敏感字符替换为此处文字(不填则替换成‘*’)</div>
        <?php  } else { ?>
            <div class='help-block'><?php  echo $data['replykeyword'];?></div>
        <?php  } ?>
    </div>
</div>
