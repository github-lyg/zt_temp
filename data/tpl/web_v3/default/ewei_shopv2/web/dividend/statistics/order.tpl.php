...
<div class="page-header">
    当前位置：<span class="text-primary">分红订单
...
<div class="page-content">
    <form action="./index.php" method="get" class="form-horizontal table-search" role="form">
        ...
        <input type="hidden" name="r" value="dividend.statistics.order" />
        <input type="hidden" name="status" value="<?php  echo $status;?>" />
        <input type="hidden" name="agentid" value="<?php  echo $_GPC['headsid'];?>" />
    ...
    "
            ...
            <td  rowspan="<?php  echo count($item['goods'])?>" style='text-align:center' >
                <?php if(cv('member.member.edit')) { ?>
                <a href="<?php  echo webUrl('member/list/detail',array('id'=>$item['mid']))?>"> <?php  echo $item['m1']['nickname'];?></a>
                <?php  } else { ?>
