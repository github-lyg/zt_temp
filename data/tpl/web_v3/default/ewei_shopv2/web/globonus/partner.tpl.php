...
<div class="page-content">
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-select">
                        <select name='partnerlevel' class='form-control  input-sm select-md' style="width:140px;">
                            ...
                            <option value='{$level[' id']}' <?php  if($_GPC['partnerlevel']==$level['id']) { ?>selected<?php  } ?>><?php  echo $level['levelname'];?></option>
                            <?php  } } ?>
                        </select>
    ...
    <div class="page-table-header">
        ...
        <div class="btn-group">
            ...
            <a class='btn  btn-default btn-operation' data-toggle='batch'
               data-href="<?php  echo webUrl('globonus/partner/partnerblack',array('partnerblack'=>1))?>"
               data-confirm='确认要设置黑名单?'>
                <i class="icow icow-heimingdan2"></i> 设置黑名单
            </a>
            <a class='btn btn-default btn-operation' data-toggle='batch'
               data-href="<?php  echo webUrl('globonus/partner/partnerblack',array('partnerblack'=>0))?>"
               data-confirm='确认要取消黑名单?'>
                <i class="icow icow-yongxinyonghu"></i> 取消黑名单
            </a>
            <a class='btn btn-default btn-operation' data-toggle='batch'
               data-href="<?php  echo webUrl('globonus/partner/check',array('status'=>1))?>" data-confirm='确认要审核通过?'>
                <i class="icow icow-shenhetongguo"></i>审核通过
            </a>
            <a class='btn btn-default btn-operation' data-toggle='batch'
               data-href="<?php  echo webUrl('globonus/partner/check',array('status'=>0))?>" data-confirm='确认要取消审核?'>
                <i class="icow icow-yiquxiao"></i>取消审核
            </a>
            <?php  } ?>
            <?php if(cv('globonus.partner.delete')) { ?>
            <a class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch-remove'
               data-confirm="确认要删除?" data-href="<?php  echo webUrl('globonus/partner/delete')?>">
        ...
        <tr>
            ...
            <td style="overflow: visible">
                <div rel="pop" style="display: flex" data-content="
                    ...
                    <a href="<?php  echo webUrl('member/list/detail',array('id' => $row['id']));?>" title='会员信息'
                       target='_blank' style="display: flex;display: -webkit-flex">
                        <?php  if(!empty($row['avatar'])) { ?>
                        <img class="radius50" src='<?php  echo tomedia($row['avatar'])?>'
                        style='width:30px;height:30px;padding1px;border:1px solid #ccc' />
                        <?php  } ?>
                        <span style="display: flex;flex-direction: column;justify-content: center;align-items: flex-start;padding-left: 5px">
                            <span class="nickname">
                                <?php  if(strexists($row['openid'],'sns_wa')) { ?>
                                <i class="icow icow-xiaochengxu" style="color: #7586db;vertical-align: middle;" data-toggle="tooltip" data-placement="top" title="" data-original-title="来源: 小程序"></i>
                                <?php  } ?>
                            ...
                            <?php  if($row['partnerblack']==1) { ?>
                                <span class="text-danger">
                                    黑名单
                                    <i class="icow icow-heimingdan1" style="color: #db2228;margin-left: 2px;font-size: 13px;"></i>
                                </span>
                            <?php  } ?>
                        </span>
                    </a>
            ...
            <td class="text-warning"><?php  echo $row['partner_froze_amount'];?></td>
            <td>
            ...
            <td>
                <span class='label
                    <?php  if($row['partnerstatus']==1) { ?>
                        label-primary
                    <?php  } else { ?>
                        label-default
                    <?php  } ?>'
                    <?php if(cv('globonus.partner.check')) { ?>
                        data-toggle='ajaxSwitch' data-confirm ='确认要<?php  if($row['partnerstatus']==1) { ?>取消审核<?php  } else { ?>审核通过<?php  } ?>?'
