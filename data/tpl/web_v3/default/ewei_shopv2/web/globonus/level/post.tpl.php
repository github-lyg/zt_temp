<?php defined('IN_IA') or exit('Access Denied');?><form action="" <?php if( ce('globonus.level' ,$level) ) { ?>action="" method="post"<?php  } ?>  class="form-horizontal form-validate" enctype="multipart/form-data">
<input type="hidden" name="id" value="<?php  echo $level['id'];?>"/>
<input type="hidden" name="r" value="globonus.level.<?php  if(empty($level['id'])) { ?>add<?php  } else { ?>edit<?php  } ?>"/>
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <button data-dismiss="modal" class="close" type="button">×</button>
            <h4 class="modal-title"><?php  if(!empty($level['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>股东等级</h4>
        </div>
        <div class="modal-body">
            <div class="form-group">
                <label class="col-sm-2 control-label must">等级名称</label>
                <div class="col-sm-9 col-xs-12">
                    <?php if( ce('globonus.level' ,$level) ) { ?>
            ...
            <div class="form-group">
                <label class="col-sm-2 control-label">等级权重</label>
                <div class="col-sm-9 col-xs-12">
                    <?php if( ce('globonus.level' ,$level) ) { ?>
                    <div class='input-group' style="width: 100%">
                        <input type="text" name="hierarchy" class="form-control" value="<?php  echo $level['hierarchy'];?>"/>
                    </div>
                    <span class='help-block'>大于零的整数，数值越大权重越高</span>
                    <?php  } else { ?>
                    <div class='form-control-static'><?php  echo $level['hierarchy'];?></div>
                    <?php  } ?>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    ...
                    <div class='input-group'>
                        <input type="text" name="bonus" class="form-control" value="<?php  echo $level['bonus'];?>"/>
                        <div class='input-group-addon'>%</div>
                    </div>
                    <span class='help-block'>支持小数点后四位</span>
                    <?php  } else { ?>
                    <div class='form-control-static'><?php  echo $level['bonus'];?>%</div>
                    <?php  } ?>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    <?php if( ce('globonus.level' ,$level) ) { ?>
                    <div class='input-group' style="width: 100%">
                        <input type="text" name="froze_amount" class="form-control" value="<?php  echo $level['froze_amount'];?>"/>
                    </div>
                    <span class='help-block'>冻结金额永久冻结在平台，大于冻结金额部分才可以提现</span>
                    <?php  } else { ?>
                    <div class='form-control-static'><?php  echo $level['froze_amount'];?></div>
                    <?php  } ?>
                </div>
            </div>
                ...
                <div class="col-sm-9 col-xs-12">
                    ...
                    <div class='input-group'>
                        ...
                        <input type="text" name="commissionmoney" class="form-control"
                               value="<?php  echo $level['commissionmoney'];?>"/>
                        <span class='input-group-addon'>元</span>
                        <?php  } ?>
                        <?php  if($leveltype==11) { ?>
                    ...
                    <span class='help-block'>股东升级条件，不填写默认为不自动升级</span>
