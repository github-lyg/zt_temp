...
<div class="fui-navbar">
    <a href="<?php  echo mobileUrl('globonus')?>" class="external nav-item <?php  if($_W['routes']=='globonus') { ?>active<?php  } ?>">
        <span class="icon icon-home"></span>
        <span class="label"><?php  echo $this->set['texts']['center']?></span>
    </a>

    <!--<a href="<?php  echo mobileUrl('globonus/bonus',array('status'=>0))?>"
       class="external nav-item <?php  if($_W['routes']=='globonus.bonus' && $_GPC['status']==0) { ?>active<?php  } ?>">
        <span class="icon icon-money"></span>
        <span class="label"><?php  echo $this->set['texts']['bonus_total']?></span>
    </a>

    <a href="<?php  echo mobileUrl('globonus/bonus',array('status'=>2))?>"
       class="external nav-item <?php  if($_W['routes']=='globonus.bonus' && $_GPC['status']==2) { ?>active<?php  } ?>">
        <span class="icon icon-process"></span>
        <span class="label"><?php  echo $this->set['texts']['bonus_lock']?></span>
    </a>-->

    <a href="<?php  echo mobileUrl('globonus/bonus',array('status'=>1))?>"
       class="external nav-item <?php  if($_W['routes']=='globonus.bonus' && $_GPC['status']==1) { ?>active<?php  } ?>">
