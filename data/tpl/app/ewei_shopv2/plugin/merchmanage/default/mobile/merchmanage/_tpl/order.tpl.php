<?php defined('IN_IA') or exit('Access Denied');?><script type="text/html" id="tpl_order">
    <%each list as item%>
        ...
        <div class="fui-list-group <%if item.merchname && item.merchid>0%>nomargin<%/if%>" data-order="<%item.id%>">
            ...
            <a class="fui-list order-list title-b" href="<?php  echo mobileUrl('merchmanage/order/detail')?>&id=<%item.id%>" data-nocache="true">
            ...
            <div class="fui-list-group-title text-right big">
                ...
                <a class="btn btn-default btn-sm" href="<?php  echo mobileUrl('merchmanage/order/detail')?>&id=<%item.id%>">查看详情</a>
            </div>
        </div>
    <%/each%>
</script>