...
<div class='fui-page fui-page-current'>
    ...
    <div class="head-menu">
        <?php  if($status==4 || $status==5) { ?>
            <a href="<?php  echo mobileUrl('merchmanage/order')?>" class="external"><i class="icon icon-rejectedorder"></i> 普通订单</a>
        <?php  } else { ?>
            <a href="<?php  echo mobileUrl('merchmanage/order', array('status'=>4))?>" class="external"><i class="icon icon-rejectedorder"></i> 维权订单</a>
        <?php  } ?>
    </div>

    <?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('merchmanage/_tpl/order', TEMPLATE_INCLUDEPATH)) : (include template('merchmanage/_tpl/order', TEMPLATE_INCLUDEPATH));?>

    <script type="text/javascript" src="../addons/ewei_shopv2/plugin/merchmanage/static/js/init.js?v=<?php  echo time()?>"></script>
    <script language="javascript">
        require(['../addons/ewei_shopv2/plugin/merchmanage/static/js/order.js'],function(modal){
