...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function main() 
	{
		...
		$psize = 10;
		$params = array( ":uniacid" => $_W["u      niacid"] );
	...
	public function setting() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			empty($saveCover) and show_json(0, "回复设置失败");
			$saveRet = $this->model->taskSave("ewei_shop_task_set", $set);
			empty($saveRet) and show_json(0, "保存失败");
			show_json(1);
		}
	...
	public function post_new() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			empty($data["title"]) and show_json(0, "请填写标题");
			$data["image"] = trim($_GPC["image"]);
			empty($data["image"]) and show_json(0, "请上传任务图");
			$data["type"] = trim($_GPC["type"]);
			empty($data["type"]) and show_json(0, "请选择任务类型");
			$taskType = $this->model->getTaskType($data["type"]);
			empty($taskType) and show_json(0, "任务类型错误");
			...
			if( is_array($data["requiregoods"]) ) 
			{
				...
				empty($requiregoods) and show_json(0, "请选择指定商品");
				$data["requiregoods"] = implode(",", $requiregoods);
			}
			$opentime = $_GPC["opentime"];
			$data["starttime"] = $opentime["start"];
			empty($data["starttime"]) and show_json(0, "请选择开始时间");
			$data["endtime"] = $opentime["end"];
			empty($data["endtime"]) and show_json(0, "请选择结束时间");
			...
			if( $data["picktype"] != "1" ) 
			{
				...
				switch( $data["stop_type"] ) 
				{
					...
					empty($data["stop_time"]) and show_json(0, "请选择截止日期");
					break;
		...
		{
			!empty($task["followreward"]) and json_decode($task["followreward"], 1);
			!empty($task["reward"]) and json_decode($task["reward"], 1);
			!empty($task["reward2"]) and json_decode($task["reward2"], 1);
			!empty($task["reward3"]) and json_decode($task["reward3"], 1);
			empty($reward["coupon"]) and $reward["coupon"] = array( );
			empty($reward2["coupon"]) and $reward2["coupon"] = array( );
			empty($reward3["coupon"]) and $reward3["coupon"] = array( );
			!empty($task["design_data"]) and json_decode(str_replace("&quot;", "'", $task["design_data"]), true);
			if( empty($task) ) 
			{
