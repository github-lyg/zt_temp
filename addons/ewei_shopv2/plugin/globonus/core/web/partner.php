...
//			$row['bonus'] = $bonus['ok'];
            $row['bonus'] = pdo_fetchcolumn('select sum(money) from' . tablename('ewei_shop_globonus_billp') . (' where uniacid=' . $_W['uniacid'] . ' and status = 1 and openid=:openid'), array(':openid' => $row['openid'])) ?: 0;
