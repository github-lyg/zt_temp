...
class Level_EweiShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $default = array(
            ...
            'bonus'        => $set['bonus'],
            'hierarchy'    => $set['hierarchy'] ?: 0,
            'froze_amount' => $set['froze_amount'] ?: '0.00'
        );
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            $data = array(
                ...
                'hierarchy'       => intval($_GPC['hierarchy']),
                'froze_amount'    => trim($_GPC['froze_amount']) ?: 0,
            );

            if (!empty($id)) {
                if ($id == 'default') {
                    $updatecontent = '<br/>等级名称: ' . $set['levelname'] . '->' . $data['levelname'] . ('<br/>分红比例: ' . $set['bonus'] . '->' . $data['bonus']) . '<br/>冻结金额: ' . $set['froze_amount'] . '->' . $data['froze_amount'];
                    $set['levelname'] = $data['levelname'];
                    $set['bonus'] = $data['bonus'];
                    $set['froze_amount'] = $data['froze_amount'];
                    $this->updateSet($set);
                    plog('globonus.level.edit', '修改股东默认等级' . $updatecontent);
                } else {
                    $updatecontent = '<br/>等级名称: ' . $level['levelname'] . '->' . $data['levelname'] . ('<br/>分红比例: ' . $level['bonus'] . '->' . $data['bonus']) . '<br/>冻结金额: ' . $level['froze_amount'] . '->' . $data['froze_amount'];
