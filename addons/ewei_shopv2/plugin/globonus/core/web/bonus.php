    ...
    public function payp()
    {
        ...
        if (!empty($member)) {
            ...
            if (is_error($result)) {
                ...
                $this->model->sendMessage($partner['openid'], array('money' => $partner['paymoney'], 'nickname' => $member['nickname'], 'type' => ($payAmount > 0) ? ($moneytype ? '微信钱包' : '余额') : "冻结分红"), TM_GLOBONUS_PAY);
            }
        } else {
