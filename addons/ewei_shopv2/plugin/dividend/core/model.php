...
if (!class_exists("DividendModel")) {
    class DividendModel extends PluginModel
    {
        ...
        public function checkOrderConfirm($orderid = "0")
        {
            ...
            $orderBonusMoney = $order["price"] + $order["deductcredit2"];
            ...
            $divedend["dividend_price"] = number_format((($orderBonusMoney - $order["dispatchprice"]) * $set["ratio"]) / 100, 2);
        ...
        public function addDividendBonus($orderId = 0)
        {
            global $_W;

            $order = pdo_fetch("select id,openid,ordersn,goodsprice,dividend,agentid,finishtime from " . tablename("ewei_shop_order") . " where id=:id and status>=3 and uniacid=:uniacid limit 1", array(":id" => $orderId, ":uniacid" => $_W["uniacid"]));
            if (empty($order)) {
                return NULL;
            }

            $orderDividend = iunserializer($order["dividend"]);
            //增加分红记录
            pdo_insert('ewei_shop_dividend_bonus', array(
                'uniacid'    => $_W['uniacid'],
                'openid'     => $order['openid'],
                'orderid'    => $order['id'],
                'bonus'      => $orderDividend["dividend_price"],
                'createtime' => time(),
            ));
        }
