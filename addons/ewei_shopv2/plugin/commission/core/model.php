...
if( !class_exists("CommissionModel") )
{
    class CommissionModel extends PluginModel
    {
        ...
        public function calculate($orderid = 0, $update = true, $order_agentid = NULL)
        {
            ...
            //增加波比（最终佣金 * 佣金比例 = 实际佣金）
            if ($set['bonusRate'] && $set['bonusRate'] > 0) {
                $rate = $rate * $set['bonusRate'] / 100;
            }
            ...
            if( 0 < $set["level"] )
            {
                foreach( $goods as &$cinfo )
                {
                    ...
                    if ($commission["commissiontype"] == 2) {
                        $orderGoods = pdo_fetch("select * from " . tablename("ewei_shop_goods") . " where uniacid=:uniacid and id=:goodsid", array(":uniacid" => $_W["uniacid"], ":goodsid" => $cinfo['goodsid']));
                        //echo "<pre>"; print_r($orderGoods); exit;
                        $price = ($cinfo["realprice"] - $orderGoods['costprice'] * $cinfo["total"]) * $rate;
        ...
        public function checkAgent($openid = "")
        {
            ...
            $parent = false;
            $mid = intval($_GPC["mid"]);
			file_put_contents(IA_ROOT . "/addons/ewei_shopv2/data/tmp/190731/m.txt", $mid);
            if( !empty($mid) )
            {
                $parent = m("member")->getMember($mid);
				file_put_contents(IA_ROOT . "/addons/ewei_shopv2/data/tmp/190731/m1.txt", print_r($parent, 1));
            }
        ...
        public function checkAgent1($openid = '')
        {
            global $_W;
            global $_GPC;
            $set = $this->getSet();
            ...
            if ($parent_is_agent && empty($member['agentid']))
            {
                ...
                {
                    $this->saveRelation($member['id'], $parent['id'], 1);
                    $dividend = pdo_fetch('select id,isheads,headsid,headsstatus from ' . tablename('ewei_shop_member') . ' where id = :id', array(':id' => $parent['id']));
                    $dividend_init = pdo_fetch('select * from ' . tablename('ewei_shop_dividend_init') . ' where headsid = :headsid', array(':headsid' => $parent['id']));
                    if (!(empty($dividend['isheads'])) && !(empty($dividend['headsstatus'])) && !(empty($dividend_init['status'])))
                    {
                        pdo_update('ewei_shop_member', array('headsid' => $parent['id']), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
                        ...
                        if (!(empty($data)))
                        {
                            ...
                            pdo_update('ewei_shop_member', array('headsid' => $parent['id']), array('id' => $ids));
                        }
                    }
...
//                        'paymoney'    => $partner['bonusmoney_send'] * $rate,
                        ...
                        'realmoney'   => $partner['bonusmoney'],
                        'paymoney'    => $partner['bonusmoney'],
                        'bonus'       => $partner['bonus'],
        ...
        public function sendMessage($openid = "", $data = array( ), $message_type = "")
        {
            ...
            if( $tm["is_advanced"] && ($tag == "commission_agent_new" || $tag == "commission_order_pay" || $tag == "commission_order_finish") )
            {
                ...
                if( $set["selfbuy"] == 1 )
                {
                    ...
                    m("notice")->sendNotice(array( "openid" => $data["orderopenid"], "tag" => $tag, "default" => $message, "cusdefault" => $text, "datas" => $datas, "plugin" => "commission" ));
