	...
	protected function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			if( !empty($id) ) 
			{
				...
				// 修改拼团营销 2019-3-1 小罗
				pdo_update("ewei_shop_groups_set", array('discount'=>$data['discount'],'headstype'=>$data['headstype'],'headsmoney'=>$data['headsmoney'],'headsdiscount'=>$data['headsdiscount']), array( "id" => $id, "uniacid" => $_W["uniacid"] ));
				// ----->
				if( !empty($ladder) ) 
				{
			...
			else 
			{
				...
				// 修改拼团营销 2019-3-1  小罗
				pdo_insert("ewei_shop_groups_set", array('discount'=>$data['discount'],'headstype'=>$data['headstype'],'headsmoney'=>$data['headsmoney'],'headsdiscount'=>$data['headsdiscount']));
				// --->
