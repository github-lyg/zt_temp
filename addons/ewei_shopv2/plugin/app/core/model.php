...
if( !class_exists("AppModel") ) 
{
    ...
    /**
		...
		public function getCodeUnlimit($params = array()) 
		{
			...
			load()->func('logging');
            logging_run('555-getCodeUnlimit:');
            logging_run($request);
		...
		public function getAuth()
		{
			...
			//cc_zhong
			load()->func('communication');
			$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/auth.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&title='.$_W['account']['name'].$uri);
		...
		global $_W;
			...
			load()->func('communication');
			$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/release2.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
			if ($request['code'] != 200) 
			{
