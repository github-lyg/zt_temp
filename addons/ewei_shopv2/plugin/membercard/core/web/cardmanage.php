...
class Cardmanage_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = "SELECT * FROM " . tablename("ewei_shop_member_card") . " where   " . $condition . " ORDER BY sort_order DESC,create_time DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize;
	...
	protected function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			if( $data["is_card_coupon"] ) 
			{
				...
				if( $count == 2 && $_GPC["couponids"][0] == $_GPC["couponids"][1] ) 
				{
					show_json(0, "开卡赠送同一张优惠券不能重复选择！");
				}
				if( $count == 3 && ($_GPC["couponids"][0] == $_GPC["couponids"][1] || $_GPC["couponids"][0] == $_GPC["couponids"][2] || $_GPC["couponids"][1] == $_GPC["couponids"][2]) ) 
				{
					show_json(0, "开卡赠送同一张优惠券不能重复选择！");
				}
	...
	public function status() 
	{
		...
		plog("membercard.cardmanage.status", (string) $operator . "会员卡 ID: " . $id . "  <br/>会员卡名称: " . $item["name"]);
