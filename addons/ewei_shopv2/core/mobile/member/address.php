...
class Address_EweiShopV2Page extends MobileLoginPage 
{
	...
	{	
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$sql = "SELECT realname,mobile,province,city,area,address FROM " . tablename("ewei_shop_member_address") . " WHERE openid = '{$_W['openid']}' AND uniacid = {$_W['uniacid']} AND id = {$id} AND deleted = 0 LIMIT 1	";
		$get_addres = pdo_fetch($sql);
		show_json(1,array('address'=>$get_addres));
	}
}
?>