...
class Util_EweiShopV2Model
{
    public function getExpressList($express, $expresssn)
    {
        ...
        if (!isset($info) || empty($info['data']) || !is_array($info['data'])) {
            ...
            // $content = $response['content'];
            // echo "<pre>";print_r(!empty($content));exit;
            // $info = json_decode($content, true);
            // $useapi = false;
            ...
            //调用查询物流轨迹
            //---------------------------------------------
            // defined('EBusinessID') or define('EBusinessID', '1641756');

            // //电商加密私钥，快递鸟提供，注意保管，不要泄漏
            // defined('AppKey') or define('AppKey', '4f322f37-b060-4a13-83e0-df80676debae');
            // //请求url
            ...
            $info=$this->getOrderTracesByJson($EBusinessID,$AppKey,$ReqURL);
            $info = json_decode($info, true);
            $useapi = false;
            // $content = iconv("utf-8", "gb2312//IGNORE", $logisticResult);
 ...
 * Json方式 查询订单物流轨迹
 */
    public function getOrderTracesByJson($EBusinessID,$AppKey,$ReqURL)
    {

        $requestData= "{'OrderCode':'','ShipperCode':'ZTO','LogisticCode':'75348572499256'}";
