...
class List_EweiShopV2Page extends WebPage 
{
	protected function orderData($status, $st) 
	{
		...
		if( !empty($list) ) 
		{
			...
			{
			    if (p('globonus') && $value['isglobonus'] != 1) {
			        $bill = pdo_fetch('select b.bonusrate from ' . tablename('ewei_shop_globonus_bill') . ' b ' . '  left join ' . tablename('ewei_shop_globonus_billo') . ' o on o.billid = b.id ' . ('  where o.orderid=:orderid limit 1'), array(':orderid' => $value['id']));

                    $value['bonusrate'] = $bill['bonusrate'];
